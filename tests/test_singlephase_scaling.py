...
def test_singlephase_scaling_with_pressure_drop(line_network):
    r1 = solve(line_network, FluidSinglePhase(1.0), PressureBC("inlet_xmin", "outlet_xmax", 1.0, 0.0), axis="x")
    r2 = solve(line_network, FluidSinglePhase(1.0), PressureBC("inlet_xmin", "outlet_xmax", 2.0, 0.0), axis="x")
...
def test_singlephase_scaling_with_viscosity(line_network):
    ...
    line_network.throat["area"] = np.sqrt(8.0 * np.pi) * np.ones(2)  # Gives g=1 for mu=1 and L=1 via generic model
    r1g = solve(line_network, FluidSinglePhase(1.0), PressureBC("inlet_xmin", "outlet_xmax", 1.0, 0.0), axis="x")
    r2g = solve(line_network, FluidSinglePhase(2.0), PressureBC("inlet_xmin", "outlet_xmax", 1.0, 0.0), axis="x")
