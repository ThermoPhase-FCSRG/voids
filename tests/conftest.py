from __future__ import annotations

import numpy as np
import pytest

import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "src"))

from voids.core.network import Network
from voids.core.sample import SampleGeometry


@pytest.fixture()
def line_network() -> Network:
    pore_coords = np.array([[0, 0, 0], [0.5, 0, 0], [1.0, 0, 0]], dtype=float)
    throat_conns = np.array([[0, 1], [1, 2]], dtype=int)
    pore = {"volume": np.array([1.0, 1.0, 1.0])}
    throat = {
        "volume": np.array([0.5, 0.5]),
        "length": np.array([1.0, 1.0]),
        "hydraulic_conductance": np.array([1.0, 1.0]),
    }
    labels = {
        "inlet_xmin": np.array([True, False, False]),
        "outlet_xmax": np.array([False, False, True]),
        "boundary": np.array([True, False, True]),
    }
    sample = SampleGeometry(bulk_volume=10.0, lengths={"x": 1.0}, cross_sections={"x": 1.0})
    return Network(throat_conns=throat_conns, pore_coords=pore_coords, sample=sample, pore=pore, throat=throat, pore_labels=labels)


@pytest.fixture()
def branched_network() -> Network:
    pore_coords = np.array([[0,0,0],[1,0,0],[2,0,0],[1,1,0],[5,5,0]], dtype=float)
    throat_conns = np.array([[0,1],[1,2],[1,3]], dtype=int)
    pore = {"volume": np.ones(5)}
    throat = {"volume": np.ones(3)*0.2, "length": np.ones(3), "hydraulic_conductance": np.ones(3)}
    labels = {
        "inlet_xmin": np.array([True, False, False, False, False]),
        "outlet_xmax": np.array([False, False, True, False, False]),
        "boundary": np.array([True, False, True, False, False]),
    }
    sample = SampleGeometry(bulk_volume=20.0, lengths={"x":2.0}, cross_sections={"x":1.0})
    return Network(throat_conns=throat_conns, pore_coords=pore_coords, sample=sample, pore=pore, throat=throat, pore_labels=labels)
